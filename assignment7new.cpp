...
void simplex_solver(vector<vector<double> > full_eqns,
  ...
  cout << "DONE" << endl;
  cout << "a: Number of Iters \nb: Leaving Vars \nc: Entering vars min delta + obj val\n"
          "d: Optimal Solution \nx: To end"
       << endl;
  while (1) {
    char ch;
    ...
    switch (ch) {
      ...
      case 'b': {
        ...
        if(index_cache[i].second[position] <= index_cache[0].first.size())
          ...
          cout << "z" << (index_cache[i].second[position] - index_cache[i].first.size())<< "\t";
      ...
      case 'c': {
        ...
        if(index_cache[i].first[position] <= index_cache[0].first.size())
          ...
          cout << "z" << (index_cache[i].first[position] - index_cache[i].first.size())<< "\t";

        cout <<"Min Ratio: "<<thetas_cache[i][position]<<endl;
        cout<<"OBJECTIVE FUNCTION = "<<table_cache[i][full_eqns.size()-1][full_eqns[0].size()-1]<<endl;
...
int main(int argc, char const *argv[]) {
  ...
  cout<<"ENTER Variable types 1 for free 0 far false"<<endl;
  int count_num = 0;
  for(int i = 0; i < r; i++)
  { 
    cin>>var_record[i];
...
vector<vector<double> > eqns_dual_only(r, vector<double>(n + 1 + dual_count, 0.0));
  for(int i = 0; i < r; i++)
  {
      int k = n;
    for(int j = 0; j < n; j++)
    ...
    eqns_dual_only[i][n+dual_count] = optimal_eq[i];
  }
  vector<double> optimal_dual_eq;
  for(int i = 0; i < n; i++)
  {
      optimal_dual_eq.push_back(eqns_only[i][n+dual_count]);
  }
  optimal_dual_eq.push_back(0.0);
  cout<<"DUAL START"<<endl;
  print2D(eqns_dual_only);
  print1D(optimal_dual_eq);
  simplex_solver(eqns_dual_only, optimal_dual_eq);
  cout<<"DUAL END"<<endl;
