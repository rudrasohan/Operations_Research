...
pair<bool, pair<double, vector<double> > > simplex_solver(
    vector<vector<double> > full_eqns, vector<double> full_obj) {
  // Initialization
  ...
  if (optim.first) {
    optim.second.first =
        eqns_final[eqns_final.size() - 1][eqns_final[0].size() - 1];
...
pair<vector<vector<double> >, pair<vector<int>, vector<int> > >
dual_simplex_solver(vector<vector<double> > full_eqns, vector<double> full_obj,
                    pair<vector<int>, vector<int> > p, int status) {
  // Initialization
  ...
  int menu;
  cout << " FOR CHECKING THE CURRENT INFO of the iteration of the dual simplex "
          "press 1 else 0"
       << endl;
  cin >> menu;
      ...
      switch (ch) {
        ...
        case 'c': {
          ...
          if (index_cache[i].first[position] <= index_cache[0].first.size())
            ...
            cout << "z" << (index_cache[i].first[position] -
                            index_cache[i].first.size())
                 << "\t";

          cout << "Min Ratio: " << thetas_cache[i][position] << endl;
          cout << "OBJECTIVE FUNCTION = "
               << table_cache[i][full_eqns.size() - 1][full_eqns[0].size() - 1]
               << endl;
...
void branch_and_bound_recurse(vector<vector<double> > full_eqns,
  ...
  if (status.first) {
    cout << "optimal value of this iter" << sol_init.second.first << endl;
...
void cutting_plane(vector<vector<double> > full_eqns, vector<double> full_obj) {
  pair<bool, pair<double, vector<double> > > sol_init =
      simplex_solver(full_eqns, full_obj);
  vector<vector<double> > frac_eq;
  ...
  do {
    ...
    if (f_flag) {
      ...
      for (int i = 0; i < current_indices.second.size(); i++) {
        // cout<<indices.second[i]<<","<<indices.first.size()<<endl;
        if (current_indices.second[i] <= current_indices.first.size()) {
          cut_final.push_back(current_tableu[i][current_tableu[0].size() - 1]);
        }
      }
      print1D(cut_final);
      cout << "OPTIM = " << current_tableu[current_tableu.size() - 1]
                                          [current_tableu[0].size() - 1]
           << endl;
      globly = current_tableu[current_tableu.size() - 1]
                             [current_tableu[0].size() - 1];
      break;
    }
    ...
    for (int i = 0; i < v.size(); i++) {
      v[i] =
          -(current_tableu[max_pose][i] - floor(current_tableu[max_pose][i]));
    }
    frac_eq.push_back(v);
    ...
    // print2D(current_tableu);
    cout << "THE FINAL" << endl;
    current_indices.second.push_back(current_indices.first.size() +
                                     current_indices.second.size() + 1);
    cout << "PRINTING TABLE" << endl;
    print_simplex_table(current_tableu, current_indices);
    cout << "STARTING DUAL" << endl;
    pair<vector<vector<double> >, pair<vector<int>, vector<int> > > final_one =
        dual_simplex_solver(current_tableu, full_obj, current_indices, 1);
    final_tableu.push_back(final_one.first);
    final_indices.push_back(final_one.second);
  } while (!f_flag);
  int i;
  while (1) {
    cout << "Enter the iter to see the fraction equation <=" << (counter - 1)
         << endl;
...
int main(int argc, char const *argv[]) {
  ...
  if (num == 1){ 
    ...
    branch_and_bound_recurse(eqns, optimal_eq, 0);
    cout << "OPTIMAL SOLUTION FROM CUTTING PLANE: " << globly << endl;
    cout << "OPTIMAL SOLUTION FROM BRANCH AND BOUND: " << global_best << endl;
    cout << "Basic Vriables from CUTTING PLANE" << endl;
    print1D(finale);
    cout << "Basic Variables form BRANCH AND BOUND" << endl;
    print1D(cut_final);
  }
  return 0;
}
