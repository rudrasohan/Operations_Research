...
void simplex_solver(vector<vector<double> > full_eqns,
  ...
  for (int i = 0; i < full_eqns.size(); i++) {
    if (full_eqns[i][full_eqns[0].size() - 2] == -1) {
      var1++;
      var2++;
    } else
      var1++;
  }
  int n_cols = full_eqns[0].size() + var2 - 1;
  vector<vector<double> > eqns_final(rows + 1, vector<double>(n_cols, 0.0));
  int k = 0;
  for (int i = 0; i < rows; i++) {
    bool ddone = false;
    for (int j = 0; j < n_cols; j++) {
      if (j < (full_eqns[0].size() - 2))
        eqns_final[i][j] = full_eqns[i][j];
      else if (j == n_cols - 1)
        eqns_final[i][j] = full_eqns[i][full_eqns[0].size() - 1];
      else {
        if ((full_eqns[i][full_eqns[0].size() - 2] == -1) &&
            (k == (j - rows + 1))) {
          eqns_final[i][j] = -1.0;
      ...
      // cout<<"ELE"<<i<<","<<j<<":"<<eqns_final[i][j]<<endl;
    }
    if (ddone) k++;
  }
  ...
  for (int i = 0; i < n_cols; i++) {
    for (int j = 0; j < rows; j++) {
      cout << i << "," << j << endl;
      eqns_final[rows][i] += M_index[j] * eqns_final[j][i];
    }
    if (i < n_cols - 1) eqns_final[rows][i] += full_obj[i];
  }
  cout << "ANS" << endl;
  print2D<double>(eqns_final);
  cout << "SOL CHECKER" << endl;
  ...
  while (1) {
    ...
    for (int i = 0; i < full_obj.size(); i++) {
      // cout<<"COMPARE "<<eqns_final[obj_row][i]<<endl;
      if ((eqns_final[obj_row][min_pos] >= eqns_final[obj_row][i]) &&
          (eqns_final[obj_row][i] < 0)) {
        min_pos = i;
        done = false;
      }
    }
    ...
    for (int i = 0; i < eqns_final.size(); i++) {
      if (eqns_final[i][min_pos] > 0) unbounded = false;
    }
    ...
    for (int i = 0; i < eqns_final.size(); i++) {
      for (int j = 0; j < eqns_final[0].size(); j++) {
        ...
        p = eqns_final[pv.first][pv.second];
        s = eqns_final[i][j];
